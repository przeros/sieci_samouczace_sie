...
def model_of_car(state, wheel_turn_angle, V, var_global):
    state = (state.x, state.y, state. car_angle)
    dt = var_global.dt
    ...
    while if_end == False:
        ...
        xsn = xn + ds * np.cos(alpha_next)  # new position of geometrical center of a car
        ...
        W = (X >= -street_length / 2) & (X <= street_length / 2) & \
            (Y <= street_width + car_width / 2) & \
            ((Y >= car_width / 2) | ((Y >= car_width / 2 - park_depth) & (X >= -place_width / 2) &
                                     (X <= place_width / 2)))
